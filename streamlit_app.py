...
with st.form('extras', clear_on_submit=True):
    ...
    #     extra_val = st.number_input(label='Value (***optional***)', min_value=0.00, max_value=100.00, step=0.01, key='extra_val')
    # else: 
    extra_val = st.number_input(label='Value (***optional***)', min_value=0.00, step=0.01, key='extra_val')
    st.session_state.discount= (0, None)
    st.session_state.add_charge= (0, None)
...
def grand_total(discount, add_charge):
    ...
    if 'Add. Charge' not in st.session_state.persons: 
        total_charge= 0
    else: 
        total_charge= st.session_state.persons['Add. Charge']

    if discount: 
        val, split_type= st.session_state.discount
        # st.write(val, split_type)
        if split_type=='Percentage':
            total_disc+= 0.01*val*st.session_state.persons['Total']
        elif split_type=='Equal split':
            total_disc+= val/len(st.session_state.persons)  
        
        st.session_state.persons['Discount']= round(total_disc,2)
        st.session_state.discount= (0, 'Percentage')
    
    if add_charge:
        # st.write('hi')
        val, split_type= st.session_state.add_charge
        if split_type=='Percentage':
            total_charge+= 0.01*val*st.session_state.persons['Total']
        elif split_type=='Equal split':
            total_charge+= val/len(st.session_state.persons)
        
        st.session_state.persons['Add. Charge']= round(total_charge, 2)
        # st.write(st.session_state.persons['Add. Charge'])
        st.session_state.add_charge= (0, 'Percentage')


    grand_total= round(st.session_state.persons['Total'] - total_disc + total_charge, 2)
...
if st.session_state.persons.empty:
    ...
    st.session_state.persons['Grand Total']= grand_total(st.session_state.discount[1], st.session_state.add_charge[1])

    if 'Discount' in st.session_state.persons.columns and 'Add. Charge' in st.session_state.persons.columns:
    ...
    # reset the index
    st.session_state.item_list.index= pd.Index(range(1, len(st.session_state.item_list)+1))
    st.session_state.persons.index= pd.Index(range(1, len(st.session_state.persons)+1))
